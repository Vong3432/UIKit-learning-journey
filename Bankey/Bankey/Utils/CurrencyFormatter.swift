...
struct CurrencyFormatter {
    ...
    private func convertDollar(_ dollarPart: Double) -> String {
        let dollarsWithDecimal = dollarsFormatted(dollarPart) // "RM929,466.00"
        let formatter = NumberFormatter()
        let decimalSeparator = formatter.decimalSeparator! // "."
        let dollarComponents = dollarsWithDecimal.components(separatedBy: decimalSeparator) // "RM929,466" "00"
        var dollars = dollarComponents.first! // "RM929,466"
        dollars.removeFirst() // "M929,466"
    ...
    private func makeBalanceAttributed(dollars: String, cents: String) -> NSMutableAttributedString {
        ...
        let rootString = NSMutableAttributedString(string: "RM", attributes: dollarSignAttributes)
